...
Explicit retry requires an existing logical notification in definitive `failed` state for the same organization, progress update, guardian, and event. The conditional `failed` to pending/submitting transition owns the retry; neither a missing failure, a submitted state, nor an ambiguous reservation can initiate a first-time retry send. Retry responses still report stored recipient truth: an existing submitted row is returned as already submitted, and a pending/uncertain row is returned as ambiguous without contacting the relay. Multi-recipient responses include reconciled counts and use a partial-result code whenever recipient outcomes differ, so a successful guardian submission is never hidden by another guardian's failure.
