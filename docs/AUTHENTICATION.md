...
The source-level cause of the misleading staging result was an inconsistent invitation error contract, not an additional complexity, Unicode-length, or whitespace rule. The password service returned the specific `PASSWORD_TOO_SHORT`, `PASSWORD_TOO_LONG`, or `PASSWORD_EQUALS_EMAIL` code, while the invitation page recognized only the obsolete aggregate `PASSWORD_POLICY` code. It consequently replaced the applicable password-policy error with the invalid-invitation state, losing the server's reason and making a policy rejection appear to be a bad link. Password-change failures were likewise collapsed into a generic message. The reset flow already preserved its specific policy codes. All creation flows now use the same client-side code-point validator, preserve the Worker's specific policy response, and keep confirmation mismatch separate from token validity.
