...
## Phase 2.7 password authentication (complete)
...
The original 600,000-iteration work factor was selected from general PBKDF2 guidance. The deployed Cloudflare Workers runtime rejects PBKDF2 iteration counts above 100,000, so new credentials use that supported maximum. Verification reads the work factor stored on the credential rather than assuming the current creation default; however, Cloudflare cannot verify preserved 600,000-iteration credentials because its runtime rejects that work factor. Migration `0005_cloudflare_password_work_factor.sql` was applied to the remote `qurantrack-staging` D1 database before the staging Worker was deployed with the required password pepper; editing the already-applied `0004` migration would not have upgraded an existing database. The original unit tests ran Web Crypto in Node/jsdom, whose implementation accepts 600,000 iterations, and therefore could not reproduce the deployed runtime's ceiling. Regression coverage now asserts the Cloudflare-compatible maximum at the Web Crypto call boundary and across password creation, change, invitation, and reset flows; this is the closest local runtime-compatible check without exercising a deployed Worker.

### Phase 2.7 staging and production status

Phase 2.7 is complete. The implementation is merged, migration `0005_cloudflare_password_work_factor.sql` was applied to the remote `qurantrack-staging` D1 database, and Worker `qurantrack-staging` was deployed with `PASSWORD_HASH_PEPPER` configured. Manual staging acceptance passed for password creation, password login, password change, password reset, invitation acceptance, first-password creation, magic-link fallback, validation boundaries, and organization/role context. Automated regression coverage is provided by the repository test suite and is distinct from manual staging acceptance. Production was not deployed, migrated, or otherwise modified as part of this closeout.
