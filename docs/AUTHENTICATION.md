...
Public password, magic-link, and reset requests use purpose-separated hashed account/IP subjects in bounded 15-minute D1 counters. Reset tokens expire after 30 minutes, are stored only as purpose-separated hashes, are single-use, and invalidate older tokens. Creating/changing a password revokes every session and rotates the current browser session while retaining a valid active organization. Reset revokes all sessions and requires a new sign-in. Parents may reuse this account foundation later; parent access is not implemented. Students remain non-login records.
